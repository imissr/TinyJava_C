...
{
    if (!item || !item->parentTable || !item->parentTable->parent)
        return NULL;
...
void itemToString(Item *item, char *buffer, int bufferSize)
{
    ...
    snprintf(buffer, bufferSize, "%s\n%s\n%s\n%s\n%s\n%s",
             item->itemKind == NOKIND ? "-" : (item->itemKind == KIND_CLASS ? "CLASS" : item->itemKind == KIND_METHOD ? "METHOD"
                                                                                    : item->itemKind == KIND_VAR      ? "VAR"
                                                                                                                      : "BLOCK"),
